...
- 明确要"性能预测 / 热力学 / 相图 / 计算 / 验证成分" → `analyst`
- 明确要"查历史数据 / 推荐成分 / 查数据库" → `data_expert`
- 明确要"生成报告 / 总结 / 最终结论 / 够了 / 结束" → `report_writer`

### 规则 2：引导挂件选择

当用户通过引导挂件做出了选择（消息中包含"用户选择了："或挂件返回格式）：
- 选择"性能预测"或"热力学计算" → `analysisExpert`
- 选择"继续查询数据" → `dataExpert`
- 选择"生成报告"、"暂时够了"或"结束" → `reportWriter`
